 ...
 */
@Repository
public class PayableDao extends HibernateDao<Payable, Long>{
	
	/** 通过父级对象主键查询下属对象列表 */
	@SuppressWarnings("unchecked")
	public List<Payable> queryPayableTreeView(Long parentId) {
	...
	/** 单个对象分页查询条件 */
	@SuppressWarnings("unchecked")
	public Page<Payable> queryPayableGridView(String query, Page<Payable> page) {
	...
	@SuppressWarnings("unchecked")
	public Page<Payable> queryPayableGridView(Long typeId, String query, Page<Payable> page) {
		List<PropertyFilter> filters = new LinkedList<PropertyFilter>();
		if (typeId != null) {
	...
	@SuppressWarnings("unchecked")
	public Page<Payable> queryPayable(String query, Page<Payable> page){
		//	sql
		...
		List<Payable> list = session.createSQLQuery(sb.toString()).addEntity(Payable.class).list();
		page.setResult(list);
	...
	@SuppressWarnings("unchecked")
	public Page<Payable> queryNonPersistenceObject(String query, Page<Payable> page){
		//	sql
		...
		SQLQuery sqlQuery = (SQLQuery) session.createSQLQuery(sb.toString()).setResultTransformer(Transformers.aliasToBean(Payable.class));// 需要绑定的非hibernate持久化对象
