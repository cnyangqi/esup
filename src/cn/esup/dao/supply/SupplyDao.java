...
public class SupplyDao extends HibernateDao<Supply, Long>{
	...
	public List<Supply> querySupplyTreeView(Long parentId) {
		Criteria criteria = getSession().createCriteria(Supply.class);
		if (parentId != null) {
	...
	/** 单个对象分页查询条件 */
	@SuppressWarnings("unchecked")
	public Page<Supply> querySupplyGridView(String query, Page<Supply> page) {
	...
	@SuppressWarnings("unchecked")
	public Page<Supply> querySupplyGridView(Long typeId, String query, Page<Supply> page) {
		List<PropertyFilter> filters = new LinkedList<PropertyFilter>();
		if (typeId != null) {
	...
	@SuppressWarnings("unchecked")
	public Page<Supply> querySupply(String query, Page<Supply> page){
		//	sql
		...
		List<Supply> list = session.createSQLQuery(sb.toString()).addEntity(Supply.class).list();
		page.setResult(list);
	...
	@SuppressWarnings("unchecked")
	public Page<Supply> queryNonPersistenceObject(String query, Page<Supply> page){
		//	sql
		...
		SQLQuery sqlQuery = (SQLQuery) session.createSQLQuery(sb.toString()).setResultTransformer(Transformers.aliasToBean(Supply.class));// 需要绑定的非hibernate持久化对象
