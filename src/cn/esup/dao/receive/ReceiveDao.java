 ...
 */
@Repository
public class ReceiveDao extends HibernateDao<Receive, Long> {

	/** 通过父级对象主键查询下属对象列表 */
	@SuppressWarnings("unchecked")
	public List<Receive> queryReceiveTreeView(Long parentId) {
		Criteria criteria = getSession().createCriteria(Receive.class);
		if (parentId != null) {
	...
	/** 单个对象分页查询条件 */
	@SuppressWarnings("unchecked")
	public Page<Receive> queryReceiveGridView(String query, Page<Receive> page) {
	...
	@SuppressWarnings("unchecked")
	public Page<Receive> queryReceiveGridView(Long typeId, String query, Page<Receive> page) {
		List<PropertyFilter> filters = new LinkedList<PropertyFilter>();
		if (typeId != null) {
	...
	@SuppressWarnings("unchecked")
	public Page<Receive> queryReceive(String query, Page<Receive> page) {
		//	sql
		...
		List<Receive> list = session.createSQLQuery(sb.toString()).addEntity(Receive.class).list();
		page.setResult(list);
	...
	@SuppressWarnings("unchecked")
	public Page<Receive> queryNonPersistenceObject(String query, Page<Receive> page) {
		//	sql
		...
		SQLQuery sqlQuery = (SQLQuery) session.createSQLQuery(sb.toString()).setResultTransformer(
				Transformers.aliasToBean(Receive.class));// 需要绑定的非hibernate持久化对象
	...
	public List<Receive> quertTop10Record() {
		// select * from receive t where t.`status`=1 order by t.id desc limit 10;

		StringBuilder sb = new StringBuilder();
		sb.append("select * from receive t where t.`status`=1 order by t.id desc limit 20");
