...
package cn.esup.dao.savingaccount;
 ...
 */
@Repository
public class SavingaccountDao extends HibernateDao<Savingaccount, Long> {

	/** 通过父级对象主键查询下属对象列表 */
	@SuppressWarnings("unchecked")
	public List<Savingaccount> querySavingaccountTreeView(Long parentId) {
		Criteria criteria = getSession().createCriteria(Savingaccount.class);
		if (parentId != null) {
	...
	/** 单个对象分页查询条件 */
	@SuppressWarnings("unchecked")
	public Page<Savingaccount> querySavingaccountGridView(String query, Page<Savingaccount> page) {
	...
	@SuppressWarnings("unchecked")
	public Page<Savingaccount> querySavingaccountGridView(Long typeId, String query, Page<Savingaccount> page) {
		List<PropertyFilter> filters = new LinkedList<PropertyFilter>();
		if (typeId != null) {
	...
	@SuppressWarnings("unchecked")
	public Page<Savingaccount> querySavingaccount(String query, Page<Savingaccount> page) {
		//	sql
		...
		List<Savingaccount> list = session.createSQLQuery(sb.toString()).addEntity(Savingaccount.class).list();
		page.setResult(list);
	...
	@SuppressWarnings("unchecked")
	public Page<Savingaccount> queryNonPersistenceObject(String query, Page<Savingaccount> page) {
		//	sql
		...
		SQLQuery sqlQuery = (SQLQuery) session.createSQLQuery(sb.toString()).setResultTransformer(
				Transformers.aliasToBean(Savingaccount.class));// 需要绑定的非hibernate持久化对象
	...
	public void initStatus() {
		// update savingaccount t set t.`status`='' where t.`status`=1

		StringBuilder sb = new StringBuilder();
		sb.append("update savingaccount t set t.`status`='' where t.`status`=1");
