 ...
 */
@Repository
public class GameDao extends HibernateDao<Game, Long> {

	/** 通过父级对象主键查询下属对象列表 */
	@SuppressWarnings("unchecked")
	public List<Game> queryGameTreeView(Long parentId) {
	...
	/** 单个对象分页查询条件 */
	@SuppressWarnings("unchecked")
	public Page<Game> queryGameGridView(String query, Page<Game> page) {
	...
	@SuppressWarnings("unchecked")
	public Page<Game> queryGameGridView(Long typeId, String query, Page<Game> page) {
		List<PropertyFilter> filters = new LinkedList<PropertyFilter>();
		if (typeId != null) {
	...
	@SuppressWarnings("unchecked")
	public Page<Game> queryGame(String query, Page<Game> page) {
		//	sql
		...
		List<Game> list = session.createSQLQuery(sb.toString()).addEntity(Game.class).list();
		page.setResult(list);
	...
	@SuppressWarnings("unchecked")
	public Page<Game> queryNonPersistenceObject(String query, Page<Game> page) {
		//	sql
		...
		SQLQuery sqlQuery = (SQLQuery) session.createSQLQuery(sb.toString()).setResultTransformer(
				Transformers.aliasToBean(Game.class));// 需要绑定的非hibernate持久化对象
